...
for b in range(nbins):
	...
	file_name = home_dir + '/STATS/SPD/FINAL_MARKOV1/SIGMA/sigma_markov1_SPD_bin%i.nc' % (b+1)
	spd_data = Dataset(file_name,'r')
	tmp = np.transpose(spd_data.variables['SPD'][:])
	SPD[1,b,:,:,:] = tmp[:,:,:nt]
...
for b in range(nbins):
	...
	for i in range(ncols):
		for j in range(nrows):
			...
			ax[k].plot(t,SPD[1,b,i,j,:],'g--',label = 'From $\sigma$')
			ax[k].plot(t,SPD[2,b,i,j,:],'r-.',label = 'From $K$')
	...
	fig_name = fig_dir + 'new_parameters_markov1_SPD_bin%i' % (b+1)
...
for b in range(nbins):
	...
	for i in range(ncols):
		for j in range(nrows):
			...
			ax[k].loglog(t,SPD[1,b,i,j,:],'g--',label = 'From $\sigma$')
			ax[k].loglog(t,SPD[2,b,i,j,:],'r-.',label = 'From $K$')
	...
	ax[0].set_title('D$_x$ (km$^{2}$)')
	ax[2].set_title('D$_y$ (km$^{2}$)')
	fig_name = fig_dir + 'new_parameters_markov1_logSPD_bin%i' % (b+1)
	plt.savefig(fig_name)
