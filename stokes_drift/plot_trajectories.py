...
traj_file = data_dir + '/1/TRAJ/UNIFORM_BINS/EOF_trajectories_1-2.nc'
traj_data = Dataset(traj_file,'r')
...
traj_file = data_dir + '/2/TRAJ/UNIFORM_BINS/EOF_trajectories_7-8.nc'
traj_data = Dataset(traj_file,'r')
tmp = np.transpose(traj_data.variables['Trajectories'][:,:,:,:200,:,:])
tmp = np.swapaxes(tmp,0,4)
tmp = np.swapaxes(tmp,1,4)
...
fig_name = fig_dir + 'eof_dispersion.png'
plt.savefig(fig_name)
plt.show()
plt.close()
