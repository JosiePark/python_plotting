...
home_dir = '/media/josiepark/Seagate Expansion Drive/PhD/INTERP/TEST_CASES/STOMMEL/'
fig_dir = '/home/josiepark/Project/PhD/PYTHON_FIGURES/INTERP/STOMMEL/'
ii = 512

# READ PSI, U AND V #

psi_file = home_dir + 'NO_RK4/stommel_psi.nc'
psi_data = Dataset(psi_file,'r')
psi = np.transpose(psi_data.variables['psi'][:])

u_file = home_dir + 'NO_RK4/stommel_u.nc'
u_data = Dataset(u_file,'r')
u = np.transpose(u_data.variables['u'][:])

v_file = home_dir + 'NO_RK4/stommel_v.nc'
v_data = Dataset(v_file,'r')
v = np.transpose(v_data.variables['v'][:])
...
plt.savefig(fig_dir + 'stommel.png')
