...
home_dir = '/media/josiepark/Seagate Expansion Drive/PhD/INTERP/TEST_CASES/STOMMEL/RK4/'
...
for i in range(runs):
	file_name = home_dir + 'stommel_rk4_bicubic_dt_ %i.nc' % dt[i]
	...
	file_name = home_dir + 'stommel_rk4_2Dcubic_dt_ %i.nc' % dt[i]
	file_data = Dataset(file_name,'r')
	tmp = np.transpose(file_data.variables['Trajectories'][:])*520./ii
	...
	file_name = home_dir + 'stommel_rk4_exact_dt_ %i.nc' % dt[i]
	file_data = Dataset(file_name,'r')
	tmp = np.transpose(file_data.variables['Trajectories'][:])*520./ii
...
plt.savefig(fig_dir + 'stommel_traj_rk4.png')
